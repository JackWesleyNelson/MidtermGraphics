...
void positionListener( float posX, float posY, float posZ,
	...
	ALfloat lis[6] = { dirX,dirY,dirZ, upX,upY,upZ };
	alListenerfv(AL_POSITION, pos);
	alListenerfv(AL_ORIENTATION, lis);
}
...
void normalKeysDown( unsigned char key, int x, int y )  {
	positionSource(sources[1], cameraX, cameraY, cameraZ);
...
int main( int argc, char *argv[] ) {
    ...
    fprintf(stdout, "[INFO]: |-------------------------------------------------------------|\n");
    fprintf(stdout, "[INFO]: |   OpenAL Version:  %40s |\n", alGetString(AL_VERSION));
    fprintf(stdout, "[INFO]: |   OpenAL Renderer: %40s |\n", alGetString(AL_RENDERER));
    fprintf(stdout, "[INFO]: |   OpenAL Vendor:   %40s |\n", alGetString(AL_VENDOR));
	fprintf(stdout, "[INFO]: \\-------------------------------------------------------------/\n");
